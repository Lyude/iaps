...
#define DRIVER_DESC "Lenovo front-screen buttons testing driver"
...
static void handle_key_event(u8 *data)
{
	switch (SKEY(data)) {
		case SKEY_TOUCH:
			report_key(KEY_PROG1);
		break;
		case SKEY_ROTATE:
			report_key(KEY_SWITCHVIDEOMODE);
		break;
		default:
...
static int __init lsrot_init_module (void) {
	...
	if (!wmi_has_guid(WMI_ACPI_EVENT_GUID)) {
		printk(KERN_ERR "%s: missing WMIACPIEvent instance\n",
		       DRIVER_NAME);
		err = -ENODEV;
		goto err_clean;
	}
	...
	if (debugfs_dir) {
		/*debugfs_create_file("power", 0644, debugfs_dir, NULL,
		                    &fops_odd_pwr);*/
	}
#endif
